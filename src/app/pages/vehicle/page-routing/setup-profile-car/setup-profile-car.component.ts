...
export class SetupProfileCarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form = this.fb.group({
      ...
      frameNumber: [
        ...
        [
          ...
          Validators.pattern(/^[A-HJ-NP-Z0-9]{1,17}$/),
        ],
      ],
      machineNumber: [
        null,
      ...
      manufacturer: [
        null,
        [Validators.required, Validators.pattern(VIETNAMESE_REGEX)],
      ],
    ...
    this.form.get('vehicleTypeId')?.valueChanges.subscribe((value: any) => {
      if (value) {
        const isTruck = this.listVehicle.find(
    ...
    this.form.get('driver.driverName')?.valueChanges.subscribe((value: any) => {
      // Tìm lái xe theo id
      const selectedDriver = this.listDriver.find(
        (driver: any) => driver.id === value
      ); // So sánh theo id
      if (selectedDriver) {
  ...
  validateYear(name: string | (string | number)[], event: Event) {
    this.validateService.validateYearOfBirth(
      this.form,
      name,
      event,
      this.minYear,
  ...
  disableIntoNetworkExpireDate = (networkExpireDate: Date): boolean => {
    ...
    return networkRegisterDate
      ? networkExpireDate <= networkRegisterDate
      : false;
  };
  ...
  createVehicle() {
    ...
    const dataForm = {
      ...
      image:
        this.fileCompressed.file && this.form.get('image')?.value
          ? this.fileCompressed.file[0].name
          : null,
    ...
    if (dataForm.driver?.driverName) {
      const selectedDriver = this.listDriver.find(
        (driver) => driver.id === dataForm.driver.driverName
      );
      if (selectedDriver) {
